...
typedef struct {																												//Definição da estrutura de um quadrado
	...
	unsigned short y;
	unsigned char face;																											//Posição Y do centro do quadrado
	joystick *control;
	pistol	*gun;
	unsigned char hp;																										//Elemento de controle do quadrado no jogo (!)	
...
void square_shot(square *element);		//Protótipo da função de movimentação de um quadrado (!)
